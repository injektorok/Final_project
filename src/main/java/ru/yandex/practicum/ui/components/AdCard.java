...
public class AdCard {
    ...
    private final SelenideElement editButtonElement = $(editButtonSelector);
    ...
    public AdCard shouldHaveEditButton() {
        try {
            editButtonElement.shouldBe(Condition.exist, Duration.ofSeconds(5));
            System.out.println("Кнопка редактирования найдена");
        } catch (Exception e) {
            System.out.println("Кнопка редактирования не найдена на карточке объявления");
        }
